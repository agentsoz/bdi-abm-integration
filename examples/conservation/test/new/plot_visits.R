...
output_names <- c("cycle_number", "number_of_visits")
...
analysis <- function(db) {
	plot_file_path <- sprintf('%snumber_of_visits.pdf', experiment_dir)
	pdf(plot_file_path)
	for (i in 1:sampleCount ) {
		...
		print(sprintf("sample number:%s, accumulated visits:%s", i, total_acc))
