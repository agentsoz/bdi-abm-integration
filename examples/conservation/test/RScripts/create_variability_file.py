...
def readVariable(curs, variable, num_cycles):
	...
	index = 0	
	for row in temp_mat:
		result_mat[index,0] = numpy.var(row)
		index += 1
...
variability_mat = numpy.zeros((num_cycles,len(variables) + 1))

rowCount = 0
for cycle in cycles:
	variability_mat[rowCount,0] = cycle[0]
	rowCount += 1

index = 1
for var in variables:
	variability_mat[:,index] = readVariable(curs, var, num_cycles)
	index += 1

numpy.savetxt(args.EXPERIMENT_DIR + "/varibility_over_time.csv", variability_mat, delimiter=",", header="cycle_number," + str(variables).strip('[]'))
