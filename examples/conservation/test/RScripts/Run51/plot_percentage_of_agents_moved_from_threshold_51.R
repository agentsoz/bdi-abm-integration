...
readData <- function(db, sampleNumber){
	...
	for (c in 1:cycles){
		...
		output[c,] = c(c,increased * 100/initially_low, decreased* 100/ initially_high)
	}
...
plot_file_path <- sprintf('%spercentage_of_agents_changed_CE_category.pdf', experiment_dir)
pdf(plot_file_path)
