...
analysis <- function(db) {
	...
	boxplot(s1_data, ylab ="Number of agents increased/decreased their CE \n in 100 auction cycles",ylim=range(c(0,100)))
	title('Initial high CE agents% = 25%')

	print(sprintf('Average number of agents increased their CE: %s', mean(s1_data[,1])))
	print(sprintf('Average number of agents decreased their CE: %s', mean(s1_data[,2])))
}
...
readData <- function(db, sampleNumber){
	...
	for (r in 1:REPLICATES){
		...
		for(col in 4:length(last_cycle)) {
			...
			if(initial_values[col] > last_cycle[col]){
		...
		output[r,] = c(increased, decreased)
	}
...
plot_file_path <- sprintf('%sbox_5.pdf', experiment_dir)
pdf(plot_file_path)
