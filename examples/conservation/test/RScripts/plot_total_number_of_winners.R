...
y_axis_params <- c("number_of_successful_bids")

# process database

plot_pair <- function(all) {
	y_label <- sprintf('number of winners')
        plot(all[,c(x_axis_param,y_axis_params[1])], xlab=x_axis_param, ylab=y_label, ylim=range(c(0,100)))
        title("linear scatter:number of winners in each category")
}


analysis <- function(db) {
	plot_file_path <- sprintf('%sall_winners.pdf', experiment_dir)
	pdf(plot_file_path)
