...
trips to and from. The only information in it that the algorithm needs is the columns "LOCALITY_N", "Type", "Count", "xcoord" and "ycoord". If the user would like to use a different csv to the example provided, these four bits of informations should have titles matching the above.
...
in the MATSim config file. In general, it is easier to keep activity labels consistent across agent types, and instead vary the "Type" locations
that they map to. At present, "home" is reserved as the first and last destination for ALL agents. This means that every agent should have their
first activity listed as "home". To make the plans more realistic, the activity "work" is coded to have double duration.
