...
# h<-y
y<-dnorm(x, mean=8, sd=3)
#y<-y/max(y)
y2<-dnorm(x, mean=15, sd=3)
#y2<-y2/max(y2)
y<-.5*y+.5*y2
#y<-y/max(y)
a<-rep("At Beach", length(x))
...
y<-dnorm(x, mean=11, sd=3)
#y<-y/max(y)
y2<-dnorm(x, mean=19, sd=2)
#y2<-y2/max(y2)
y<-.5*y+.5*y2
#y<-y/max(y)
...
df<-rbind(df1,df2)
ggplot(df,aes(x=Hour,y=Proportion, color=Activity))+ggtitle("Activity start times") +labs(y="Proportion", x="Hour of day") +geom_line()
```
