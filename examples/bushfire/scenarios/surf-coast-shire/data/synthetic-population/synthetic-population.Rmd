...
#Probability of each activity occuring (Resident)
w=0.8
b=0.3
s=0.5
...
x<-seq(from=0, to=24,length.out=960)
y<-dnorm(x, mean=8, sd=.05)

a<-rep("Leave to Work", length(x))
df3<-data.frame(Hour=x,Density=y, Activity=a)
w<-y
...
a<-rep("Leave to Beach", length(x))
df1<-data.frame(Hour=x,Density=y, Activity=a)
b<-y

y<-dnorm(x, mean=11, sd=2)
#y<-y/max(y)
y2<-dnorm(x, mean=19, sd=1.5)
#y2<-y2/max(y2)
y<-(.5*y+.5*y2)
#y<-y/max(y)
a<-rep("Leave to Shops", length(x))
df2<-data.frame(Hour=x,Density=y, Activity=a)

df<-rbind(df1,df2)
ggplot(df,aes(x=Hour,y=Density, color=Activity))+ggtitle("Activity start times") +labs(y="Density", x="Hour of day") +scale_x_continuous(breaks=c(0,6,12,18,24)) +geom_line()
```
