...
#function to generate counts based on probability
generate_count<-function(prob,number)
  
{
 count<-prob*number
 decimal<-count-floor(count)
 M<-apply(count,2,which.max)
 # decimal[decimal==0]<-1
 # m<-apply(decimal,2,which.min)
 # Floor<-floor(count[cbind(m,seq_along(m))])
 # Ceiling<-ceiling(count[cbind(M,seq_along(M))])
 count<-floor(count)
 count[cbind(M,seq_along(M))]<-count[cbind(M,seq_along(M))]+colSums(decimal)
 # count[cbind(m,seq_along(m))]<-Floor
return(count)    
}
#timestep
i=2
...
#new prob matrix
prob<-sweep(count,2,colSums(count),'/') 
  
p_resident<-apply(prob,2,cumsum)
...
RESIDENTS[[h]]<-res
count<-count-res
if(sum(colSums(res))>12){break}
}

check<-lapply(RESIDENTS,colSums)
