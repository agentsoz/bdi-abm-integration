...
```{r echo=FALSE, fig.width=5, fig.height=3}
durations<-matrix(c(2,8,2,4,4), nrow=1, dimnames=list(c("hours"), c("home","work","beach","shops","other")))
show(durations)
```
...
#matrix showing when durations from previous timesteps end
x=matrix(1:12,5,12,byrow = T)-dur
x[x<0]<-0
x[,1]=1
A<-matrix(1,1,12)
...
#   # if (sum(x[,h])>0)
#    # {
#     a=matrix(rep(x[,h],12),5,12)/matrix(1:12,5,12,byrow = T)
#     a[a!=1]=0
#     b=rowSums(a*p_resident)
#     c=sum(a*p_resident)
#     A[h]=c*A[h]
#     A
#     p_resident[,h]=(p_resident[,h]-rowSums(p_resident[,1:(h-1)]))/c
#     p_resident
#     #x[,h][x[,h]==m]<-0
#    # }
#   
